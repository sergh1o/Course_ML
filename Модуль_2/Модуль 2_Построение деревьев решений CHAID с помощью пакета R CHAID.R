...
predprob_hold <- predict(chd, holdout, type = "prob")
result <- data.frame(holdout, result=predvalue_hold)
result <- data.frame(result, result=predprob_hold)

## 2.3.3. Работа с матрицей ошибок

# снижаем пороговое значение спрогнозированной
# вероятности положительного класса до 0.3
table(holdout$churn == "Уходит", 
      predict(chd, holdout, type = "prob")[, "Уходит"] >= 0.30)

# повышаем пороговое значение спрогнозированной
# вероятности положительного класса до 0.7
table(holdout$churn == "Уходит", 
      predict(chd, holdout, type = "prob")[, "Уходит"] >= 0.70)
...
# формируем обучающую, контрольную
# и тестовую выборки
...
# гиперпараметров на объединенных данных
